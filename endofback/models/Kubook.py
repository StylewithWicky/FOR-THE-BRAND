...
class Kubook(SQLModel, table=True):
    ...
    trip:Optional["Matrip"] = Relationship(back_populates="kubooks" , sa_relationship_kwargs={"lazy": "selectin"})
    sherehe_id: int | None = Field(default=None, foreign_key="sherehe.id")
    sherehe: Optional['Sherehe'] = Relationship(back_populates="kubooks" , sa_relationship_kwargs={"lazy": "selectin"})
