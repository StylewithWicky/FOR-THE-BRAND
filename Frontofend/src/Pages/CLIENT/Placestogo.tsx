...
export default function PlacesToGo() {
  ...
  return (
    <div ref={containerRef} className="bg-[#0b0c0e] text-white min-h-screen font-sans selection:bg-amber-500 overflow-x-hidden">
      ...
      >
        <video autoPlay loop muted playsInline className="absolute inset-0 w-full h-full object-cover brightness-[0.35] pointer-events-none">
      ...
      <section className="py-10 md:py-16 max-w-7xl mx-auto px-6">
        ...
        {loading ? (
          <div className="text-center py-24 border border-dashed border-white/10 rounded-[30px] bg-white/[0.01]">
            <p className="font-mono text-xs text-amber-500 uppercase tracking-widest animate-pulse">TuLiA NikUpAnGE...</p>
          </div>
        ) : error ? (
          <div className="text-center py-24 border border-red-500/20 rounded-[30px] bg-red-500/[0.02]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6 md:gap-8 items-start">
            {sanctuaries.map((sanctuary, index) => {
              ...
              return (
                <div key={sanctuary.id || sanctuary.name} className={`group border border-white/5 rounded-[30px] md:rounded-[40px] overflow-hidden bg-white/[0.01] hover:bg-white/[0.02] transition-all duration-500 ${isLarge ? 'lg:col-span-8' : 'lg:col-span-4'}`}>
                  ...
                  <div className="p-6 md:p-8 space-y-4">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed font-light line-clamp-3">{sanctuary.desc}</p>
                    
                    <a 
                      href={`https://wa.me/254111782146?text=Hi YOLO Connect! I'm interested in booking the ${encodeURIComponent(sanctuary.name)} package.`}
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="block w-full text-center py-4 mt-6 bg-amber-500 text-black text-[10px] font-bold uppercase tracking-[0.2em] rounded-full hover:bg-white transition-all duration-300"
                    >
                      Secure Your Spot
                    </a>
                  </div>
